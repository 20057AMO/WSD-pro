...
| Opencode Studio | `tests/opencode-studio.test.ts` | Full baked roster (20 agents / 10 skills / 8 slash commands), agent+skill+command CRUD lifecycles, kebab-name + traversal rejection, frontmatter-required command writes, config merge preserves keys / rejects junk, version-info shape (against running container) |
...
- **Baked roster (v2 — full SDLC coverage, English prompts)**: 20 subagents in `opencode/agents/*.md` — planning/design: architect · ux-designer; implementation: frontend-developer · backend-developer · db-expert; quality: code-reviewer · test-writer · refactorer · debugger · perf-optimizer; security: security-auditor (read-only) + pentester (active, bash-only with hard limits); APIs/docs: api-designer · doc-writer; ops: devops-engineer · release-manager; platform: wsd-expert; languages: python-expert · golang-expert · rust-expert. Reviewers are `edit: deny`+`bash: deny`, implementers `allow/allow`, pentester bash-only
