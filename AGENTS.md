...
| Snapshot automation | `tests/project-snapshots-auto.test.ts` | Scheduled server-side backups: defaults + partial-config validation, capture-now (archive on disk + lastSnapshotAt stamp), **retention pruning to `keep`**, download (gzip+filename) → delete → 404, restore filed snapshot into a brand-new project (files/notes/description), member-viewer read-only vs outsider 403 matrix, and pure `computeDueSnapshots` interval rules (import-free `snapshots-schedule.ts`, like janitor-core) |
