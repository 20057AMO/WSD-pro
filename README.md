...
The installer detects the local IP (LAN or Tailscale), installs dependencies, builds the workspace image, starts the backend + IDE containers, sets up Caddy with self-signed TLS, optionally installs Ollama with `qwen2.5-coder`, and prints the access URLs.
