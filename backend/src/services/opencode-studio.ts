...
/** Pull `description` / `mode` out of markdown frontmatter for listings. */
function frontmatterMeta(raw: string): { description: string; mode?: string } {
  const m = /^---\r?\n([\s\S]*?)\r?\n---/.exec(raw);
  if (!m) return { description: '' };
  const meta: { description: string; mode?: string } = { description: '' };
